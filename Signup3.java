...
public class Signup3 extends JFrame implements ActionListener{
    ...
    // ---------------------
    // ADDED **ONLY VALIDATION**
    // ---------------------
    private boolean validateInputs(String account_type, String facility) {

        // Account type must be selected
        if (account_type == null || account_type.trim().equals("")) {
...
long part1 = 4000000000000000L; 
long part2 = Math.abs(ran.nextLong() % 100000000000000L); 
String card_number = String.format("%016d", part1 + part2);

// Generate 4-digit PIN
int pinNum = ran.nextInt(9000) + 1000; 
String pin = String.valueOf(pinNum);
        ...
        try{
            if(ae.getSource()==b1){
                ...
                // 🔥 ONLY DATA VALIDATION ADDED
                if (!validateInputs(account_type, facility)) {
                    return;
                }

                Conn c1 = new Conn();
                String q1 = "insert into signup3 values('"+form_number+"','"+account_type+"','"+card_number+"','"+pin+"','"+facility+"')";  
                String q2 = "insert into login values('"+form_number+"','"+card_number+"','"+pin+"')";
